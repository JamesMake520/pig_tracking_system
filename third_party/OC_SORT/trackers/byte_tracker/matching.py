...
def linear_assignment(cost_matrix, thresh):
    ...
    mask = matched_cost <= thresh
    matches = np.stack([row_ind[mask], col_ind[mask]], axis=1) if mask.any() else np.empty((0, 2), dtype=int)
    unmatched_a = tuple(set(range(cost_matrix.shape[0])) - set(row_ind[mask].tolist()))
    unmatched_b = tuple(set(range(cost_matrix.shape[1])) - set(col_ind[mask].tolist()))
    return matches, unmatched_a, unmatched_b
